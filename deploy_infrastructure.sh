...
logstash_setup() {
    ...
    echo "export LS_HOME=$(pwd)/logstash-$logstash_version" >> ~/.bash_profile
    source ~/.bash_profile
    echo "LS_HOME=$LS_HOME is set up "
    echo "Copy $ls_config_name to $LS_HOME/config"
    cp -v $config_repo_name/configs/logstash/ $LS_HOME/config
...
elastic_setup() {
    ...
    echo "export ES_HOME=$(pwd)/elasticsearch-$elastic_version" >> ~/.bash_profile
    source ~/.bash_profile
...
grafana_setup() {
    ...
    echo "export GRAFANA_HOME=$(pwd)/grafana-$grafana_version" >> ~/.bash_profile
    source ~/.bash_profile
    echo "GRAFANA_HOME=$GRAFANA_HOME is set up"
    echo "Provision datasources"
    cp -v $config_repo_name/configs/grafana/provisioning/datasources/datasource.yaml $GRAFANA_HOME/conf/provisioning/datasources/
    echo "Provision dashboards"
    cp -r -v $config_repo_name/configs/grafana/provisioning/dashboards/* $GRAFANA_HOME/conf/provisioning/dashboards/
